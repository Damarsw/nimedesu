...
def load_json(filename):
  if not os.path.exists(filename):
    # Jika file belum ada, otomatis buat file JSON kosong agar tidak error
...
def get_iframe_links_with_retry(episode_url, max_retries=2):
  for attempt in range(max_retries + 1):
    ...
    try:
      ...
      server_links = driver.find_elements(
          By.CSS_SELECTOR, "div.mirrorstream ul li a[data-content]"
      )
        ...
        try:
          ...
          WebDriverWait(driver, 10).until(
              EC.presence_of_element_located((
                  By.CSS_SELECTOR,
                  "div.responsive-embed-stream iframe, div#pembed iframe",
              ))
          )
          ...
          if video_src and video_src not in seen_urls:
            ...
            raw_video_servers.append(
                {"resolution": resolution_text, "video_url": video_src}
            )
          time.sleep(0.5)
        except:
          continue
  ...
  try:
    ...
    if response.status_code == 200:
      ...
      if synopsis_section:
        synopsis_texts = [
            p.text.strip()
            for p in synopsis_section.find_all("p")
            if p.text.strip()
        ]
        sinopsis_text = "\n\n".join(synopsis_texts)
      ...
      if eps_section:
        for li in eps_section.find_all("li"):
          ...
          if a_tag:
            ...
            ep_url = a_tag["href"]
            if (
                "batch" not in ep_title.lower()
                and "batch" not in ep_url.lower()
            ):
              episode_list.append(
                  {"episode_title": ep_title, "episode_url": ep_url}
              )
...
def run_auto_updater():
  print(
      f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Memulai pengecekan"
      " pembaruan otomatis..."
  )
  ...
  super_map = {
      item.get("url") or item.get("link"): item
      for item in db_super_lengkap
  ...
  for anime in db_final:
    ...
    if "ongoing" in status or not status:
      ...
      stored_eps_list = (
          stored_eps_entry.get("episodes", []) if stored_eps_entry else []
      )
      ...
      if len(web_episodes) > len(stored_eps_list):
        ...
        for ep in web_episodes:
          if ep["episode_url"] not in stored_ep_urls:
            print(
                "  -> Mengambil link streaming via Selenium untuk:"
                f" {ep['episode_title']}"
            )
