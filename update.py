...
DB_EPISODE_BARU_FILE = "data_episode_baru.json"
...
def api_episode_baru():
    return jsonify(load_json(DB_EPISODE_BARU_FILE))
...
# ==========================================
# MODUL 1: FORMAT SERVER & SELENIUM STREAMING
# ==========================================
...
# ==========================================
# MODUL 2: SCRAPING DETAIL, INFOZINGLE & SINOPSIS
# ==========================================
def scrape_anime_details_from_web(anime_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Mengambil info dari div class 'infozingle' (Sinkronisasi Bot Infozingle)
            ...
            if info_section:
                for p in info_section.find_all("p"):
                    ...
                    if len(text) > 1:
                        key = text[0].strip()
                        val = ":".join(text[1:]).strip()
                        details[key] = val

            # Mengambil Sinopsis dari div class 'sinopc'
            sinopsis_text = ""
            ...
            if synopsis_section:
                ...
                sinopsis_text = "\n\n".join(synopsis_texts)
            details["sinopsis"] = sinopsis_text

            # Mengambil Episode List (Pembersihan Batch otomatis)
        ...
        print(f"Gagal mengambil detail anime dari {anime_url}: {e}")
    return {}


# ==========================================
# LOGIKA UTAMA BOT AUTO-UPDATE (24 Jam / Interval)
# ==========================================
def run_auto_updater():
    ...
    # TAHAP 1: Cek Anime Baru dari Halaman Utama Otakudesu
            ...
            for item in soup.select("ul.chivsrc li"):
                ...
                if a_tag:
                    ...
                    if url and url not in existing_final_urls:
                        ...
                        # Gabungkan detail infozingle & sinopsis jika ada
                        new_anime_entry.update(details)
    ...
    # TAHAP 2 & 3: Cek Episode Baru & Pengambilan Link Streaming Selenium
    ...
    for anime in db_final:
        ...
        # Lengkapi data infozingle/sinopsis jika kosong pada data lama
        if "Judul" not in anime or not anime.get("sinopsis"):
            web_details = scrape_anime_details_from_web(url)
            if web_details:
                anime.update(web_details)
                has_updates = True
    ...
    if has_updates:
        save_json(DB_ANIME_FINAL, db_final)
        print("\n[SUKSES] Sinkronisasi bot selesai dan file diperbarui!")
    else:
        print("\n[-] Tidak ada pembaruan baru ditemukan.")


if __name__ == "__main__":
    # Menjalankan Flask di background menggunakan Thread agar API aktif bersamaan dengan bot
    t = threading.Thread(target=run_flask)
    t.daemon = True
    t.start()

    print("=== BOT AUTO-UPDATE & API SERVER DIMULAI ===")
    INTERVAL_DETIK = 86400  # Pengecekan otomatis setiap 24 jam sekali
    while True:
        run_auto_updater()
